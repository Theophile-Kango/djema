        ...
        describe 'POST/api/v1/auth' do
            context 'when signup params is valid' do 
                ...
                it 'returns client in authentication header' do
                    expect(respnse.headers['client']).to be_present  
                end

                it 'retuns expiry in authentication header' do 
                ...
                it 'creates new user' do
                    expect{
                        post @sign_up_url, params: @signup_params.merge({email: "test@example.com"}.to change(User, :count).by(1))
                    }
                end
            end
            ...
            context 'when signup params is invalid' do 
                ...
                it 'returns unprocessable entity 422' do 
                    expect(respnse.status).to eq 422
                end
            end
        end
    end
